...
Deep state license data is available for **nine states** today. Texas, New York, and Pennsylvania are live per-query scrapes; the rest are full daily mirrors. Any other state returns `status: "unsupported_state"` on live keys — we never fabricate data we don't have.
