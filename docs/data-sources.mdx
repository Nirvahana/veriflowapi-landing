...
Deep state license data is available for **eleven states** today. Texas, New York, Pennsylvania, and Ohio are live per-query scrapes; the rest are full mirrors (refreshed daily, or monthly for California). Any other state returns `status: "unsupported_state"` on live keys — we never fabricate data we don't have.
