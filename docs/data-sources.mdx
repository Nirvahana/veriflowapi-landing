...
Deep state license data is available for **ten states** today. Texas, New York, Pennsylvania, and Ohio are live per-query scrapes; the rest are full daily mirrors. Any other state returns `status: "unsupported_state"` on live keys — we never fabricate data we don't have.
...
For states not yet mirrored — including California, which is on our roadmap — VeriflowAPI returns NPPES and OIG data, which covers federal credential status nationwide. State-level license status for additional states is added based on demand.
