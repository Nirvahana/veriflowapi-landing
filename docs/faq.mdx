...
VeriflowAPI screens providers in all 50 US states via NPPES (identity/NPI) and OIG (federal exclusions), with deep state-level license data for **eleven states**: Texas, Florida, Illinois, Washington, Colorado, Connecticut, Alabama, New York, Pennsylvania, Ohio, and California.
...
Yes, for federal NPI and OIG screening (nationwide). For full state-level license status — including expiry date and disciplinary history — we currently support eleven states: Texas, Florida, Illinois, Washington, Colorado, Connecticut, Alabama, New York, Pennsylvania, Ohio, and California. Other states return `status: "unsupported_state"`. Additional states are added based on customer demand.
