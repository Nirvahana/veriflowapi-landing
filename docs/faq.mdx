...
VeriflowAPI screens providers in all 50 US states via NPPES (identity/NPI) and OIG (federal exclusions), with deep state-level license data for **ten states**: Texas, Florida, Illinois, Washington, Colorado, Connecticut, Alabama, New York, Pennsylvania, and Ohio.

California is on the roadmap; nursing/pharmacy expansion and additional states are added based on demand. Email support@veriflowapi.com to request priority support for a specific profession.
...
Yes, for federal NPI and OIG screening (nationwide). For full state-level license status — including expiry date and disciplinary history — we currently support ten states: Texas, Florida, Illinois, Washington, Colorado, Connecticut, Alabama, New York, Pennsylvania, and Ohio. Other states return `status: "unsupported_state"`. Additional states are added based on customer demand.
